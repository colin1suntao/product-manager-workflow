...
async def _load_template_context(
    ...
    try:
        ...
        if tmpl:
            ...
            return [*context, template_context], template_info
...
async def _process_coordinator_response(
    ...
    """调用 coordinator 处理消息，构建 assistant 消息及 token 用量"""
...
async def send_message(
    ...
    # 处理消息并构建 assistant 消息
    assistant_message, token_usage = await _process_coordinator_response(
...
async def get_messages(
    ...
    return {
        "messages": [_serialize_message(m) for m in paged],
        "total": total,
        "page": page,
        "page_size": page_size,
    }
